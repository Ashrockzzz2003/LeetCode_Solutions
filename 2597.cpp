...
private:
    ...
    void generatePowerSet(vector<int>& nums, int i, vector<int>& buffer, int& count, int k) {
        if (isBeautiful(buffer, k)) count++;

        for(int j = i; j < nums.size(); j++) {
            buffer.push_back(nums[j]);
            generatePowerSet(nums, j + 1, buffer, count, k);
            buffer.pop_back();
        }
    }
public:
    int beautifulSubsets(vector<int>& nums, int k) {
        ...
        generatePowerSet(nums, 0, buffer, count, k);
